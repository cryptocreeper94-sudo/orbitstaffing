...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/talent-exchange/billing/checkout", async (req: Request, res: Response) => {
    try {
      ...
      const baseUrl = process.env.SITE_BASE_URL || 'http://localhost:5000';
    ...
    try {
      ...
      if (base58Pattern.test(query)) {
        return res.json({
          ...
          result: {
            address: query,
            explorerUrl: `${process.env.TRUSTLAYER_BASE_URL || 'https://dwtl.io'}/api/provenance/verify/${query}`,
        ...
        await emailService.sendEmail({
          ...
          html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #1a1a2e; color: #fff; padding: 40px; border-radius: 16px;">
              <img src="${process.env.SITE_BASE_URL || 'https://orbitstaffing.io'}/orbit-logo.png" alt="Orbit" style="height: 40px; margin-bottom: 24px;" />
...
export function registerPayCardRoutes(app: Express) {
  ...
  app.post("/api/release-manager/register", ecosystemAuth, async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        const trustLayerBaseUrl = process.env.TRUSTLAYER_BASE_URL || 'https://dwtl.io';
        blockchainResult = {
  ...
  app.get("/api/admin/ecosystem/performance", requireMasterAdmin, async (req: Request, res: Response) => {
    try {
      ...
      const appConfigs: Record<string, { name: string; url: string; ownership: string; color: string; status: string }> = {
        ...
        'trustvault': { name: 'Trust Vault', url: 'trustvault.studio', ownership: '100% Jason', color: 'blue', status: 'connected' },
        'thevoid': { name: 'THE VOID', url: 'enterthevoid.io', ownership: '100% Jason', color: 'purple', status: 'connected' },
        'verdara': { name: 'Verdara', url: 'verdara.io', ownership: '100% Jason', color: 'lime', status: 'connected' },
  ...
  app.post("/api/partner/stripe/onboard", requireMasterAdmin, async (req: Request, res: Response) => {
    try {
      ...
      const host = req.get('host') || new URL(process.env.SITE_BASE_URL || 'http://localhost:5000').host;
