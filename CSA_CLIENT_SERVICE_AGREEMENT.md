# ORBIT Staffing OS - Client Service Agreement (CSA)

**Effective Date:** November 22, 2025  
**Last Updated:** November 22, 2025

---

## 1. PARTIES & DEFINITIONS

**ORBIT Staffing OS** ("Provider")  
A comprehensive staffing management platform provided by DarkWave Studios LLC.

**Client** ("You")  
Any staffing agency, enterprise, parent company, or organization using ORBIT's platform. This includes:
- Independent staffing agencies
- Multi-location staffing operations
- Parent companies consolidating multiple staffing agencies
- Franchise holders
- Enterprise divisions managing temporary workforce

**Workers** ("Temporary Staff")  
Individuals placed through Client's staffing operations via ORBIT platform.

**Placement**  
Any temporary worker assignment through ORBIT from Client to end-user company.

---

## 2. WHAT ORBIT PROVIDES

### 2.1 Core Platform Services
✓ **CRM & Worker Management**  
- Worker profile management with encrypted SSN storage
- Client company records and contacts
- Complete placement history and notes
- Compliance documentation tracking

✓ **Payroll & Payment Processing**  
- Automated payroll calculation (hourly/salary)
- State-specific tax withholding (TN/KY and others)
- Overtime automation and tracking
- Payment processing and direct deposit
- Comprehensive payroll reports

✓ **Assignment Management**  
- Job posting and distribution
- Worker-to-job matching and assignment
- Real-time availability tracking
- Bulk operations for events (250-300+ workers)
- Assignment tracking and history

✓ **Time & Attendance**  
- GPS-verified clock in/out (200-300ft geofencing)
- Timesheet tracking and approval workflows
- Mobile app for worker access
- Attendance reporting

✓ **Invoicing & Billing**  
- Auto-generated client invoices from assignments
- Customizable invoice templates
- Payment tracking
- Revenue reporting

✓ **Compliance & Automation**  
- State-specific compliance rules (TN/KY pre-configured)
- I-9 requirement tracking
- Prevailing wage calculations
- Audit trails for all changes
- Automatic compliance alerts

✓ **Flexible Billing Model**  
- Monthly subscription tiers ($199-599/mo)
- Revenue share option (2% of placement revenue)
- Free switching every 6 months between models
- Perpetual franchise licensing available

✓ **Multi-Agency Hub (Enterprise)**  
- Consolidate up to 10+ staffing agencies into one dashboard
- Parent company visibility into all agencies
- Centralized reporting and analytics
- Make decisions across 1, 5, or all agencies
- Unified compliance oversight

✓ **Support & Updates**  
- Email support (48-hour response)
- Platform updates and bug fixes included
- Feature request submission and tracking
- 1-year customization period for franchises/enterprise

---

## 3. CLIENT OBLIGATIONS

### 3.1 Payment Terms
- **Monthly Subscriptions:** Payment due on or before the 1st of each month
- **Revenue Share Model:** Monthly invoicing based on reported placement revenue
- **Franchise Licenses:** One-time payment upon signature
- **Late Payment:** 1.5% monthly interest on payments overdue by 30+ days
- **Currency:** All payments in USD

### 3.2 Data Accuracy & Management
Client agrees to:
- Provide accurate worker information (name, SSN, address, tax withholding)
- Maintain current client company contact information
- Keep job details and placement terms accurate
- Update worker status when placements end
- Notify ORBIT of any data discrepancies within 5 business days

### 3.3 Compliance & Legal Responsibility
Client is responsible for:
- All hiring and employment compliance (I-9, EVerify, background checks)
- State/federal wage and hour law compliance
- Workers compensation insurance requirements
- Independent contractor vs. employee classification
- Worker safety and OSHA requirements
- Discrimination and harassment prevention

**ORBIT is NOT responsible for:**
- Compliance with Client's specific state regulations (though we provide tools)
- Worker classification decisions
- Wage rate determinations
- Hiring discrimination claims
- Worker safety on client job sites

### 3.4 Proper Platform Usage
Client agrees to:
- Use the platform only for lawful staffing operations
- Not misrepresent worker qualifications or status
- Not attempt to bypass the system or access unauthorized data
- Report any security vulnerabilities immediately
- Maintain login security and not share credentials

### 3.5 Multi-Agency Consolidation (Enterprise Clients)
If consolidating multiple staffing agencies under one ORBIT account:
- Client takes on responsibility for all consolidated agencies
- Client is liable for compliance across all agencies
- Master account holder controls all agency data
- All revenue reporting aggregates to master account

---

## 4. WORKER OBLIGATIONS

### 4.1 Information Accuracy
Workers agree to:
- Provide accurate personal information (name, SSN, address)
- Report correct tax withholding preferences
- Disclose all relevant work authorizations and licenses
- Keep contact information current

### 4.2 Placement Compliance
Workers agree to:
- Accept assignments in good faith
- Perform work safely and professionally
- Report actual hours worked accurately
- Use mobile app for proper clock in/out
- Not claim hours not actually worked
- Comply with client company rules and safety requirements

### 4.3 GPS & Attendance Verification
Workers agree to:
- Use GPS verification on mobile app for clock in/out
- Understand that clock in requires physical presence at job location (200-300ft radius)
- Accept that falsified attendance is grounds for immediate termination
- Allow accurate time tracking for payroll

### 4.4 Confidentiality
Workers agree to:
- Not share SSN or tax information with other workers
- Keep client company information confidential
- Not solicit jobs/workers outside the ORBIT system
- Not misrepresent qualifications or experience

---

## 5. DATA SECURITY & PRIVACY

### 5.1 Data Protection
ORBIT commits to:
- Encrypt all SSN and tax data at rest and in transit
- Never share personal data with third parties (except for payroll processing)
- Maintain audit trails of all data access
- Comply with GDPR, CCPA, and state privacy laws
- Regular security updates and penetration testing

### 5.2 Data We Collect
- Worker: Name, SSN, address, phone, tax withholding, work history
- Client: Company name, contacts, locations, contract terms
- Placements: Job details, pay rates, hours, GPS location, attendance

### 5.3 Data Retention
- **Active accounts:** Data retained as long as account is active
- **Closed accounts:** Data retained for 7 years for tax/compliance purposes
- **Upon request:** We can anonymize or delete personal data (except tax records)
- **Compliance:** Tax and payroll data retained per IRS requirements (7+ years)

### 5.4 Breach Notification
If personal data is compromised:
- ORBIT notifies Client within 24 hours
- ORBIT notifies workers within 48 hours
- Full investigation and remediation report within 30 days
- ORBIT covers notification costs

---

## 6. SERVICE LEVELS & SUPPORT

### 6.1 Uptime Guarantee
- **Standard Tier:** 99% uptime (14.4 min/month downtime allowed)
- **Enterprise Tier:** 99.5% uptime with dedicated support
- Planned maintenance: Announced 48 hours in advance, during off-hours

### 6.2 Support Response Times
- **Critical (System Down):** 2-hour response, 4-hour resolution target
- **High (Major Feature Broken):** 24-hour response, 48-hour resolution target
- **Medium (Minor Issues):** 48-hour response
- **Low (Feature Requests):** 5 business days

### 6.3 Support Includes
- Email support (standard)
- Phone support (1 call/month for standard, unlimited for enterprise)
- Bug fixes and patches
- Platform updates
- Feature request review

### 6.4 Support Excludes
- Custom training (billable at $150/hour)
- Custom development (billable separately)
- Integration work beyond API (billable separately)
- Legal or tax advice

---

## 7. PRICING & PAYMENT TERMS

### 7.1 Monthly Subscription Model
| Tier | Price | Workers | Features |
|------|-------|---------|----------|
| Startup | $199/mo | Up to 50 | Basic CRM, payroll, invoicing |
| Growth | $599/mo | Up to 500 | Full automation, unlimited clients |
| Enterprise | Custom | 1000+ | White-label, API, dedicated support |

### 7.2 Revenue Share Model
- **2% of monthly placement revenue**
- No monthly minimum
- Free to switch to Fixed model every 6 months
- $299 fee for additional switches

### 7.3 Franchise License
- **One-time fee:** $15,000-25,000 (depends on tier)
- **Perpetual ownership:** No recurring fees after purchase
- **White-label:** Complete custom branding
- **Support included:** 12 months of customization and support
- **Optional annual support after Year 1:** $2,000/year

### 7.4 Multi-Agency Hub
- **Base platform cost:** Per agency (Startup/Growth/Enterprise tier)
- **Hub dashboard:** Included in any base tier
- **Consolidation:** Manage up to 10+ agencies from one account
- **Cross-agency reporting:** Real-time consolidated dashboards

### 7.5 Billing & Invoicing
- **Monthly subscriptions:** Invoiced on or before 1st of month
- **Auto-renewal:** Continues month-to-month unless cancelled
- **Cancellation:** 30 days notice required
- **No refunds:** Subscription fees are non-refundable
- **Prorated changes:** Tier changes prorated to your billing cycle

---

## 8. TERMINATION & TRANSITION

### 8.1 Termination by Client
- **30 days notice required**
- **No cancellation fees** for monthly subscriptions
- **Data export:** 14 days to download all data in standard formats
- **Franchise licenses:** Non-refundable, but Client retains perpetual license

### 8.2 Termination by ORBIT
ORBIT may terminate without notice if:
- Payment is 60+ days overdue
- Client violates terms (fraud, security breach, illegal activity)
- Client repeatedly uses platform for unauthorized purposes

Otherwise:
- 30 days notice and opportunity to cure
- Client has 14 days to export data
- Customer's data access preserved for 60 days for data retrieval

### 8.3 Data After Termination
- Client can export all data for 14 days after termination
- After 14 days, data is anonymized or deleted
- Tax/payroll records retained 7 years per IRS requirements
- Backups deleted after 30 days

---

## 9. LIABILITY & DISCLAIMERS

### 9.1 ORBIT's Liability Limits
**ORBIT's total liability is limited to:**
- Monthly subscriptions: 3 months of fees paid
- Revenue share: 3 months of fees paid
- Franchise licenses: Not applicable (perpetual, one-time purchase)
- All other claims: $0 (excluded)

### 9.2 EXCLUDED DAMAGES
ORBIT is NOT liable for:
- Loss of revenue or profits
- Loss of data (except when caused by ORBIT breach)
- Indirect, incidental, or consequential damages
- Third-party claims (worker lawsuits, etc.)
- Delays or failures due to third-party services

### 9.3 WARRANTY DISCLAIMER
ORBIT provides the platform "AS-IS" without warranties for:
- Fitness for a particular purpose
- Merchantability
- Uninterrupted service
- Error-free operation
- Third-party service reliability

### 9.4 Client Indemnification
Client agrees to indemnify ORBIT from:
- Worker lawsuits or claims
- Wage and hour violations
- Discrimination or harassment claims
- Data breaches caused by Client's negligence
- Violations of law by Client's operations

---

## 10. COMPLIANCE & LEGAL

### 10.1 Regulatory Compliance
- ORBIT complies with FDCPA, EEOC, OSHA, DOL regulations
- Client is responsible for their own regulatory compliance
- ORBIT provides tools but not legal advice
- Client should consult attorney for compliance questions

### 10.2 Worker Classification
- Client determines worker classification (employee vs. contractor)
- ORBIT doesn't classify workers—Client does
- ORBIT provides tracking tools but not classification guidance
- Client liable for misclassification consequences

### 10.3 Tax & Payroll Obligations
- ORBIT calculates taxes based on Client-provided W-4/W-5 information
- Client remains responsible for accuracy of tax filings
- ORBIT provides reports; Client responsible for filing with IRS/state
- ORBIT not responsible for Client's tax errors

### 10.4 Independent Contractor Status
- ORBIT is a service provider, not an employer
- No employment relationship between ORBIT and Client
- Client retains all hiring/firing authority
- Workers classified by Client, not by ORBIT

---

## 11. CHANGES TO TERMS

ORBIT may update this agreement with:
- **30 days notice** for price changes
- **60 days notice** for material changes to services
- **Immediate effect** for security/compliance updates
- **Client's right:** Terminate within 30 days if terms are unfavorable

---

## 12. DISPUTE RESOLUTION

### 12.1 Informal Resolution
- Contact ORBIT at support@orbitstaffing.net
- Provide detailed explanation of dispute
- ORBIT responds within 5 business days
- Good faith negotiation for 30 days

### 12.2 Arbitration
If unresolved:
- Binding arbitration (not court)
- Single arbitrator (not jury)
- Costs split 50/50
- Held in Tennessee (jurisdiction of DarkWave Studios LLC)
- Governed by Tennessee law

### 12.3 Small Claims
Either party can pursue claims under $5,000 in small claims court.

---

## 13. WORKER CONVERSION & BUYOUT POLICY

### 13.1 Worker Arrangement Types
ORBIT supports flexible engagement models to accommodate diverse staffing needs:

| Arrangement Type | Definition | Duration | Typical Use |
|------------------|-----------|----------|-------------|
| **Day Labor** | Single-day assignments, no commitment | 1 day | Event staffing, emergency fill-ins |
| **Weekly** | Recurring assignments, week-to-week | 7 days (repeating) | Regular but flexible positions |
| **Part-Time** | 3-5 days/week ongoing | Ongoing | Semi-regular workforce |
| **Semi-Permanent** | 6+ months intended tenure, no conversion plan | 6+ months | Long-term temp workforce |
| **Permanent** | Intended path to full-time employment | Until conversion | Pipeline for permanent hiring |

**Worker Status Definition:**
- Each assignment is tagged with an arrangement type
- Worker can have multiple arrangement types with different clients simultaneously
- Arrangement type determines conversion eligibility and buyout policies

### 13.2 Permanent Conversion Policy (Full-Time Hiring)

**Industry Standard - Modified for ORBIT:**

When a Client intends to hire a Worker as a **full-time permanent employee**, the following buyout policy applies:

#### **Payment Requirement BEFORE Direct Hire:**
- **Less than 480 hours:** No ORBIT buyout fee (Client can hire directly)
- **480-960 hours (6-12 months):** $2,000 buyout fee required to ORBIT
- **960+ hours (12+ months):** $4,000 buyout fee required to ORBIT

**Hours Calculation:**
- Count all hours worked through ORBIT for ANY Client, regardless of assignment type
- Minimum wage = 1 hour
- Exclude unpaid break time

**Example 1 - Mike's Agency:**
- Worker: "Jake"
- Total ORBIT hours: 420 hours (4 months)
- Direct hire request from Client A: **NO FEE** (under 480 hours)
- ORBIT: Acknowledges, removes worker from system, no payment required

**Example 2 - Mike's Agency:**
- Worker: "Sarah"
- Total ORBIT hours: 650 hours (8 months)
- Direct hire request from Client B: **$2,000 BUYOUT** required
- Process: Client B sends buyout fee to ORBIT, Sarah transitions to full-time employee
- ORBIT: Receives payment, releases worker, updates records

**Example 3 - Mike's Agency:**
- Worker: "Marcus"
- Total ORBIT hours: 1,200 hours (15 months, semi-permanent)
- Client C wants to hire Marcus full-time: **$4,000 BUYOUT** required
- Justification: Marcus has deep knowledge of Client C's operations; long-term relationship

#### **Why This Structure:**
✓ **Protects ORBIT:** Compensation for losing recurring revenue stream  
✓ **Protects Client:** Clear, predictable costs for converting temps to full-time  
✓ **Protects Worker:** Ensures they transition to stable employment with proper vetting  
✓ **Incentivizes Permanent Assignments:** Clients can commit longer = better for all parties  

### 13.3 Conversion Restrictions & Conditions

**Clients CANNOT bypass this policy by:**
- Hiring worker for "personal services" outside ORBIT system
- Creating fake "break" periods to reset hour counters
- Using contractor classification to avoid conversion fees
- Direct solicitation of workers without approval

**Penalties for Violation:**
- First violation: Warning + $5,000 penalty
- Second violation: Automatic account suspension
- Third violation: Permanent account termination + legal action

**ORBIT Enforcement:**
- GPS verification data proves hours actually worked
- Audit trails show all hour calculations
- Regular compliance checks for large clients
- Workers can report unreported violations

### 13.4 Worker Conversion Workflow

**Step 1: Client Intent**
- Client notifies ORBIT: "We want to hire [Worker] full-time"
- ORBIT pulls total hours, conversion status, fee (if applicable)

**Step 2: Worker Approval**
- Worker must explicitly consent to conversion
- Worker can decline (remains on ORBIT or leaves)
- ORBIT confirms worker wants full-time role

**Step 3: Management Approval**
- For "Permanent" arrangement type: ORBIT approves (automated)
- For other types: Client's ORBIT administrator confirms worker eligible
- ORBIT checks for contractual restrictions

**Step 4: Fee Payment (if required)**
- Client pays buyout fee directly to ORBIT (invoice issued)
- Payment must clear before conversion is final
- ORBIT sends confirmation, worker released

**Step 5: Final Release**
- Worker removed from ORBIT system
- Client receives offboarding documentation
- Worker's data archived (7-year retention)

### 13.5 Semi-Permanent & No-Conversion Arrangements

**Semi-Permanent Workers (No Conversion Path):**
- Worker explicitly classified as temporary indefinitely
- No buyout applies if direct hire attempted
- Client can hire directly anytime without ORBIT fee
- **Use Case:** Seasonal workers, event staff, specific short-term needs

**Restriction:** If Client uses "Semi-Permanent" to avoid conversion fees, ORBIT can:
- Reclassify to "Permanent" after 12 months
- Apply retroactive buyout fees ($4,000)
- Charge late fees (1% per month)

**Worker Protection:** Semi-Permanent workers told upfront—no path to full-time employment through ORBIT.

### 13.6 Competing Offers & Multiple Clients

**If Worker Gets Job Offers From Multiple Clients:**

Client A offers $18/hour full-time (480 hours worked)  
Client B offers $20/hour full-time (600 hours worked)

**Scenario:**
- Worker wants to work for Client B (higher pay)
- Client B pays $2,000 buyout (for 600 hours)
- Worker releases from Client A, starts Client B
- Fair process, no disputes

**ORBIT Role:**
- Don't referee; process whichever conversion comes first
- Once converted, worker cannot re-enter ORBIT for that Client
- Worker can work for different Client through ORBIT if needed

### 13.7 Client Liability for Bypass Attempts

If Client attempts to hire Worker outside ORBIT system without:
1. Obtaining ORBIT approval, OR
2. Paying required buyout fee

**Client is liable for:**
- $5,000 penalty (first offense) + buyout fee owed
- $10,000 penalty (second offense) + legal fees
- Account termination (third offense)
- Damages to ORBIT platform (lost recurring revenue)

**ORBIT's Right to Pursue:**
- Pursue claim in arbitration (per Section 12)
- Workers can testify re: unreported hours/work
- GPS data and timesheets are admissible evidence

### 13.8 Worker Protections During Conversion

**Workers Cannot Be Penalized For:**
- Requesting conversion to full-time employment
- Declining a direct hire offer
- Remaining on ORBIT longer than expected
- Reporting conversion violations

**Workers ARE Protected:**
- All hours worked count toward conversion (no hour "forgetting")
- Overtime, bonuses, commissions count as hours worked
- Conversion fees don't come out of worker pay
- Worker remains on ORBIT until final approval

---

## 14. MULTI-AGENCY & ENTERPRISE ADDENDUM

### 13.1 Hub Consolidation
When Client consolidates multiple staffing agencies:

**Master Account Holder ("Parent"):**
- Single point of contact for billing and support
- Access to all child agencies' data
- Ability to create company-wide policies
- Unified reporting across all agencies
- Payment responsible for all child accounts

**Child Agencies:**
- Can operate independently or under parent policies
- Independent logins with parent oversight
- Separate payroll but consolidated reporting
- Parent can override decisions (if agreed in writing)

### 13.2 Consolidated Reporting
ORBIT provides:
- Real-time dashboard showing all agencies
- Revenue, margins, worker utilization across all agencies
- Compliance status for all locations
- Consolidated tax and payroll reporting
- Custom reports by agency or consolidated

### 13.3 Cross-Agency Decisions
Parent company can:
- Set billing terms across multiple agencies
- Apply compliance rules uniformly
- Adjust worker rates across agencies (if contractually permitted)
- Access payroll across all agencies
- Generate unified reporting

**Limitations:**
- Cannot override individual agency's direct client contracts
- Cannot unilaterally change worker pay (must comply with existing rates)
- Must respect state/federal minimums
- Cannot access confidential client information of other agencies (unless consolidated)

### 13.4 Separate Billing (Optional)
If consolidated but wanting separate invoices:
- Each agency receives individual invoice
- Parent account still manages all data access
- Master reporting available at parent level
- Support works through parent account

---

## 15. CONTACT & SUPPORT

**ORBIT Staffing OS Support**  
Email: support@orbitstaffing.net  
Phone: [Support Number - TBD]  
Website: orbitstaffing.net

**Billing Questions:**  
billing@orbitstaffing.net

**Security/Privacy Concerns:**  
security@orbitstaffing.net  
DarkWave Studios LLC  
TN, USA

---

## 15. SYSTEM DISCLAIMER & HUMAN ENVIRONMENT

### 15.1 Automated System with Human Management
**ORBIT is a hybrid system - automated where possible, human-managed where necessary.**

ORBIT Staffing OS operates as follows:
- **Automated:** Scheduling, payroll calculations, invoicing generation, compliance tracking, GPS verification
- **Human-Managed:** Invoice disputes, payment exceptions, worker complaints, compliance exceptions, fraud investigations
- **Manager Approval:** Staffing Agency managers approve all assignments, invoices, and conversions

### 15.2 Worker Integrity Dependency
**This system depends on worker honesty and integrity:**

Workers must:
- ✓ Clock in/out honestly via GPS at job location
- ✓ Report accurate hours worked
- ✓ Not claim hours not actually worked
- ✓ Not share GPS or login credentials
- ✓ Report suspicious activity immediately
- ✓ Maintain professional conduct with clients

ORBIT provides tools (GPS, audit trails, timesheets), but **cannot guarantee compliance if workers act dishonestly**. System is only as good as worker integrity.

### 15.3 Invoicing Dispute Resolution
**What happens if there's a dispute about hours/work documented:**

**Scenario 1: Worker claims 40 hours, GPS shows 25 hours**
- GPS verification is authoritative source
- Timesheet examined for discrepancies
- Manager reviews assignment notes
- **Resolution:** Payroll reflects GPS-verified hours
- Worker can appeal if they believe GPS error occurred

**Scenario 2: Client claims worker didn't show up, GPS shows worker present**
- GPS data takes precedence
- Client contact notes reviewed
- Manager investigates with both parties
- **Resolution:** Invoice generated based on GPS verification
- If client disputes, arbitration required

**Scenario 3: Technical glitch causes GPS error**
- Workers and clients can report GPS outages
- Manager reviews all available evidence (photos, client confirmation, worker statement)
- ORBIT makes good-faith resolution
- **ORBIT's commitment:** We will resolve disputes fairly and quickly (within 3 business days)

### 15.4 ORBIT's Commitment
**On our part:**

✓ We will investigate all disputes thoroughly  
✓ We will review GPS data, timesheets, and client notes  
✓ We will make good-faith decisions based on available evidence  
✓ We will not penalize workers or clients for honest disputes  
✓ We will resolve disputed invoices within 3 business days  

**However:**

❌ We cannot compensate for lost revenue due to worker no-shows  
❌ We cannot override GPS verification without overwhelming evidence  
❌ We cannot refund invoices already paid without clear fraud  
❌ We cannot guarantee 100% accuracy (this is human-dependent work)  

### 15.5 Staffing Industry Reality
**Understanding the nature of temporary workforce management:**

Staffing is inherently unpredictable:
- Workers sometimes call in sick or don't show
- Clients sometimes cancel assignments last-minute
- Hours sometimes vary day-to-day
- Availability is fluid and changeable
- Workers seek permanent positions and leave
- Clients' needs fluctuate

**ORBIT helps manage this chaos through:**
- Real-time scheduling and availability tracking
- GPS verification to prevent fraud
- Automated backup worker notifications
- Quick replacement assignment system
- Fair dispute resolution process

**ORBIT cannot eliminate this reality.** We provide the best tools and management, but staffing remains a dynamic, human-dependent business.

### 15.6 Worker Employment Verification
**ORBIT workers are temporary contractors, not employees.** This means:

- ✓ Workers are free to work for other staffing agencies
- ✓ Workers can decline assignments without penalty
- ✓ No guaranteed minimum hours
- ✓ No benefits (health insurance, 401k, PTO)
- ✓ Can be removed from platform for misconduct

**Workers should:**
- Always verify they're communicating with legitimate ORBIT staff
- Cross-reference manager names and contact info
- Check email addresses (official: @orbitstaffing.net)
- Never send payment upfront or provide personal financial info
- Report suspicious requests to support@orbitstaffing.net

### 15.7 System Managers & Escalation
**For disputes or issues:**

1. **First Level:** Assignment manager (handles scheduling, assignment issues)
2. **Second Level:** Compliance manager (handles payment, invoicing disputes)
3. **Third Level:** Operations director (handles major complaints, fraud)
4. **Final Level:** CEO/Owner (handles legal disputes, policy decisions)

**Manager limitations:**
- Managers cannot change system code or architecture
- Managers cannot override CEO decisions
- Managers cannot create new pricing tiers
- Managers cannot grant admin access (CEO only)
- Managers enforce policy but don't create it

---

## 16. ACCEPTANCE

By using ORBIT Staffing OS, you accept these terms:
- Check the box during signup acknowledging CSA
- Provide valid contact information
- Agree to all terms as written
- Agree to provide accurate data

**This agreement is binding upon both parties.**

---

## SIGNATURE PAGE (Digital Acceptance Required)

**Client Acknowledgment:**

By clicking "I Accept" during signup or by initiating use of ORBIT Staffing OS:

☐ I have read and understand this Client Service Agreement  
☐ I agree to all terms and conditions  
☐ I acknowledge my obligations as Client  
☐ I accept responsibility for compliance  
☐ I authorize the account holder to commit our organization  

**Date Accepted:** _______________  
**Email:** _______________  
**Company:** _______________  
**Name:** _______________  
**Title:** _______________

---

**END OF CLIENT SERVICE AGREEMENT**

Version 1.0 - Effective November 22, 2025

This agreement is a binding legal contract. By using ORBIT Staffing OS, you acknowledge you have read, understood, and agree to be bound by all terms.
