...
export const vehicles = pgTable(
  "vehicles",
  {
    ...
    status: varchar("status", { length: 50 }).default("active"), // active, maintenance, damaged
    currentAssignmentId: varchar("current_assignment_id"),
    createdAt: timestamp("created_at").default(sql`NOW()`),
    updatedAt: timestamp("updated_at").default(sql`NOW()`),
  },
  (table) => ({
    tenantIdx: index("idx_vehicles_tenant").on(table.tenantId),
    vehicleNumberIdx: index("idx_vehicles_number").on(table.vehicleNumber),
...
export const vehicleAssignments = pgTable(
  "vehicle_assignments",
  {
    ...
    mechanic: varchar("mechanic").references(() => users.id),
    maintenanceDriver: varchar("maintenance_driver").references(() => users.id),
    safetyRepresentative: varchar("safety_representative").references(() => users.id),
  ...
  (table) => ({
    ...
    supervisorIdx: index("idx_assignments_supervisor").on(table.supervisorId),
    driverIdx: index("idx_assignments_driver").on(table.maintenanceDriver),
  })
);
...
export const workOrders = pgTable(
  "work_orders",
  {
    ...
    tenantId: varchar("tenant_id").notNull().references(() => companies.id),
    vehicleId: varchar("vehicle_id").notNull().references(() => vehicles.id),
    maintenanceDriverId: varchar("maintenance_driver_id").notNull().references(() => users.id),
    workOrderNumber: varchar("work_order_number", { length: 100 }).notNull(),
    description: text("description"),
    actionType: varchar("action_type", { length: 50 }).notNull(), // jump_start, gas, inflate_tire, oil_change, coolant, other
  ...
  (table) => ({
    tenantIdx: index("idx_work_orders_tenant").on(table.tenantId),
    vehicleIdx: index("idx_work_orders_vehicle").on(table.vehicleId),
    driverIdx: index("idx_work_orders_driver").on(table.maintenanceDriverId),
    statusIdx: index("idx_work_orders_status").on(table.status),
    workOrderNumberIdx: index("idx_work_orders_number").on(table.workOrderNumber),
  })
);
...
export const safetyReports = pgTable(
  "safety_reports",
  {
    ...
    tenantId: varchar("tenant_id").notNull().references(() => companies.id),
    vehicleId: varchar("vehicle_id").notNull().references(() => vehicles.id),
    safetyRepId: varchar("safety_rep_id").notNull().references(() => users.id),
    reportDate: date("report_date").notNull(),
    safetyIssues: jsonb("safety_issues"), // array of issue strings
    status: varchar("status", { length: 50 }).default("normal"), // normal, warning, critical
    corporateNotification: jsonb("corporate_notification"), // JSON with notification info
    createdAt: timestamp("created_at").default(sql`NOW()`),
    updatedAt: timestamp("updated_at").default(sql`NOW()`),
  },
  (table) => ({
    tenantIdx: index("idx_safety_reports_tenant").on(table.tenantId),
    vehicleIdx: index("idx_safety_reports_vehicle").on(table.vehicleId),
    repIdx: index("idx_safety_reports_rep").on(table.safetyRepId),
    statusIdx: index("idx_safety_reports_status").on(table.status),
    reportDateIdx: index("idx_safety_reports_date").on(table.reportDate),
  })
);
...
// ========================
export const corporateUpdates = pgTable(
  "corporate_updates",
  {
    id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
    tenantId: varchar("tenant_id").notNull().references(() => companies.id),
    title: varchar("title", { length: 255 }).notNull(),
    content: text("content").notNull(),
    affectedVehicles: jsonb("affected_vehicles"), // array of vehicle IDs
    priority: varchar("priority", { length: 50 }).default("low"), // low, medium, high
    releaseDate: date("release_date").notNull(),
    createdAt: timestamp("created_at").default(sql`NOW()`),
    updatedAt: timestamp("updated_at").default(sql`NOW()`),
  },
  (table) => ({
    tenantIdx: index("idx_corporate_updates_tenant").on(table.tenantId),
    priorityIdx: index("idx_corporate_updates_priority").on(table.priority),
    releaseDateIdx: index("idx_corporate_updates_date").on(table.releaseDate),
  })
);

export const insertCorporateUpdateSchema = createInsertSchema(corporateUpdates).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export type InsertCorporateUpdate = z.infer<typeof insertCorporateUpdateSchema>;
export type CorporateUpdate = typeof corporateUpdates.$inferSelect;
